...
export default function EditProfile({user, loggedInUser}) {
    ...
    const HandleSave = async () => {
      ...
      if(editInfo){
        await axios.patch(`http://loaclhost:5000/userUpdates/${user?.email}`, editInfo)
        setOpen(false);
      }
    }
      ...
      >
        <Box sx={style} className='modal'>
          ...
          <form className='fill-content'>
              <TextField className='text-field' fullWidth label='Name' is='fullWidth' variant='filled' onChange={(e)=> 
                setName(e.target.value)} defaultValue={loggedInUser[0]?.name ? loggedInUser[0]?.name : ''} />
              <TextField className='text-field' fullWidth label='Bio' is='fullWidth' variant='filled' onChange={(e)=> 
                setBio(e.target.value)} defaultValue={loggedInUser[0]?.bio ? loggedInUser[0]?.bio : ''} />
              <TextField className='text-field' fullWidth label='Location' is='fullWidth' variant='filled' onChange={(e)=> 
                setLocation(e.target.value)} defaultValue={loggedInUser[0]?.location ? loggedInUser[0]?.location : ''} />
              <TextField className='text-field' fullWidth label='Website' is='fullWidth' variant='filled' onChange={(e)=> 
...
// dlf building no. 14 tower D, Recruitment Center 
// Ashu Kumar Mishra 
// CV Team 
